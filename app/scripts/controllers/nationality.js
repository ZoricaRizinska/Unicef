...
/**
 * @ngdoc function
 * @name unicefApp.controller:NationalityCtrl
 * @description
 * # NationalityCtrl
 * Controller of the unicefApp
 */
angular.module('unicefApp')
  .controller('NationalityCtrl', function($scope, ngTableParams, $http, $route) {
    ...
    $scope.deleteItem = function(id) {
      ...
      $http.delete('http://localhost:2361/api/Nationalities/' + id).
      success(function(data, status, headers, config) {
        for (var i = $scope.data.length - 1; i >= 0; i--) {
          if ($scope.data[i].NationalityId == id) {
    ...
    $scope.updateItem = function(item) {
      ...
      $http({
        method: 'PUT',
        data: item,
        url: "http://localhost:2361/api/Nationalities/" + item.NationalityId
      }).
      success(function(data, status, headers, config) {
        console.log(status);
      }).
    ...
    $scope.insertItem = function(description) {
      ...
      $http({
        method: 'POST',
        data: item,
        url: "http://localhost:2361/api/Nationalities"
      }).
      success(function(data, status, headers, config) {
