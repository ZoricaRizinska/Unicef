...
angular.module('unicefApp')
  .controller('HealtstatustypeCtrl', function($scope, ngTableParams, $http, $route) {
    ...
    $http.get('http://localhost:2361/api/HealthStatusTypes').
    ...
    $scope.deleteItem = function(id) {
      ...
      $http.delete('http://localhost:2361/api/HealthStatusTypes/' + id).
      success(function(data, status, headers, config) {
    ...
    $scope.updateItem = function(item) {
      ...
      $http({
        method: 'PUT',
        data: item,
        url: "http://localhost:2361/api/HealthStatusTypes/" + item.HealthStatusTypeId
      }).
      success(function(data, status, headers, config) {
        console.log(status);
      }).
    ...
    $scope.insertItem = function(description) {
      ...
      $http({
        method: 'POST',
        data: item,
        url: "http://localhost:2361/api/HealthStatusTypes"
      }).
      success(function(data, status, headers, config) {
        console.log(data.HealthStatusTypeId);
        $route.reload();
